...
6. 比例字段：财务指标中的百分比（毛利率、费用率等）输出百分比数值（如 63.5），unit 填 "%"；持股比例输出标准小数（如 0.2356）。
7. 每个字段必须附带来源章节名称和原文证据片段。
8. 输出必须是合法的 JSON，不要包含任何解释性文字 ，不要 markdown 代码块标记。
...
11. JSON 字符串值内禁止出现原始换行符（字面量 Enter/换行），所有多行文本必须使用 \\n 转义，或直接用空格代替换行。
"""

ISSUER_PROFILE_PROMPT = """请从以下招股说明书文本中抽取【发行人基础信息】。

需要抽取的字段（尽力查找，确实找不到再设为 null）：
...
- registered_address: 注册地址。在"概览"或"发行人基本情况"章节的基本信息表格中，搜索字段名类似为"注册地址"、"注册地"的行，取该行对应的完整地址。
  注意：必须是法定注册地址，不得填写"生产经营地址""办公地址""主要经营场所""主要生产场地""住所"等。如果表格中同时出现"注册地址"和"生产经营地址"，只取"注册地址"对应的值。
...
- registered_address 必须提取法定注册地址，不得填写生产经营地址或办公地址
...
关于 controlling_shareholder 的抽取规则：
- 直接持股和间接持股需要分开记录，通过 direct_or_indirect 字段区分。
- 如果同一控股股东既有直接持股又有间接持股，必须拆分为两条独立记录，一条 direct_or_indirect 为"直接"，另一条为"间接"，shareholding_ratio 分别填写各自的持股比例。
- 间接持股通常表现为"通过 XX 公司持有""XX 公司持有……（XX 为其全资/控股子公司）"等描述。
- direct_or_indirect 取值只能是"直接"或"间接"，不得填其他内容。
...
需要抽取的字段为列表，每个元素包含：
- field_name: 字段名称
- field_scope: 字段口径/来源（如 合并利润表、合并资产负债表、现金流量表、研发投入指标、管理层分析、募投项目预算 等）
...
【必选指标——必须从合并利润表/合并资产负债表/合并现金流量表中提取】：
- 营业收入（合并利润表）
- 净利润（合并利润表）
- 扣非净利润（合并利润表）
- 研发费用（合并利润表）
...
【募投预算口径——如文本来自募集资金运用章节，需额外提取】：
- 研发费用（各募投项目的研发费用预算，field_scope 填"募投项目预算"，chapter 填对应项目所在章节）
- 建设投资（各募投项目的建设投资，field_scope 填"募投项目预算"）
- 若同一个项目名称下有多条研发费用记录（不同项目），分别独立输出，通过 chapter 字段区分

注意：
- 金额必须拆分为数值和单位。如 "5,700.00 万元" → value=5700.0, unit="万元"。
- value 必须是纯数字（如 5700.0），禁止输出 {"value": 5700, "unit": "万元"} 这样的对象。
- 比例/百分比指标（毛利率、费用率等）输出百分比数值（如 63.5），不要输出小数（如 0.635），unit 填 "%"。
- 表格中的多个期间数据都要抽取，每个期间作为一条独立记录。
- 同名指标在不同口径下要分别输出（field_scope 不同），不要合并。
...
【重要过滤规则——以下情况不得抽取】：
- 文本仅说明"不存在""无""未发生""不涉及"某类事项（即否定性声明），例如：
  "报告期内不存在行政处罚"、"无重大诉讼"、"发行人不存在对外担保"、
  "未受到任何行政处罚"、"不存在重大违法违规"等——这类内容不抽取。
- 只有实际发生的具体事项（有当事人、金额、日期或具体情节描述）才需要抽取。
- 特别注意，如有关联交易，需要抽取出来， 通常情况下招股说明中会在”公司治理与独立性“大章的”关联方及关联交易“小节中描述关联交易的内容。

注意：
- 只抽取报告期内确实发生的、对发行人有实质影响的合规事项。
- 如果文本中对某类合规事项只有否定性声明，对应数组元素不输出。
- 输出必须是 JSON 数组，若无符合条件的事项则输出空数组 []。
- 输出合法 JSON，不要 markdown 代码块标记。
